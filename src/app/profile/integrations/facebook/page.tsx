...
export default function FacebookDetails(props: any) {

    const { currentUser, deleteFbLogin, isFBLoginDeleting,
    ...
    const fbDetails = useMemo(() => {
        let fbObj: any = {}
        if (currentUser?.company?.facebooks?.length > 0) {
            fbObj.name = currentUser?.company?.facebooks[0].commi_fb_page_name;
            fbObj.id = currentUser?.company?.facebooks[0].id;
        }
        return fbObj;
    }, [currentUser]);
            ...
            >
                <Box sx={{ flex: 1, width: '100%' }}>
                    ...
                    >
                        <Card>
                            <Box sx={{ mb: 1 }}>
                                <Typography level="title-md">Facebook Account Details</Typography>
                            </Box>
                            <Divider />
                            <Stack
                                direction="row"
                            ...
                            <CardOverflow sx={{ borderTop: '1px solid', borderColor: 'divider' }}>
                                <CardActions sx={{ alignSelf: 'flex-end', pt: 2 }}>
                                    <Button
                                        ...
                                        loading={isFBLoginDeleting}
                                        onClick={() => {
                                            deleteFbLogin(fbDetails?.id)
                                        }}>
                                        Delete
                                    </Button>
                                </CardActions>
