...
export default function ContactTable({
    ...
    const pagination = useMemo(() => {
        ...
        return <Box sx={{
            ...
            // className='flex flex-row items-center absolute ml-10' 
            sx={{
                flexDirection: 'row',
                display: 'flex',
                alignItems: 'center',
        ...
        <React.Fragment>
            ...
            >
                ...
                <FormControl sx={{ flex: 1 }} size="sm">
                    <FormLabel>Search for contacts</FormLabel>
                    <Input size="sm"
                        ...
                        >
                            <thead>
                                ...
                                >
                                    ...
                                    <th style={{ width: 90, padding: '12px 6px' }}>Customer</th>

                                    <th style={{ width: 280, padding: '12px 6px' }}>Category</th>
                                ...
                                <tbody>
                                    {[...rows].sort(getComparator(order, 'createdAt')).map((row) => (
                                        <tr key={row.id}>
                                            ...
                                            <td>
                                                <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                                                    ...
                                                    <IconButton
                                                        ...
                                                        color="neutral"
                                                        size="sm"
                                                        onClick={() => makeCall(row)}
                                                        loading={isCallMaking && callId === row.id}
                                                    >
                                                        <Call />
                                                    </IconButton>
                                                    <AvatarWithOrigin origin={row?.origin} />
                                                    <div>
                                                        <Typography level="body-xs">{row?.name ? row?.name : row.origin === 'page' ? 'Facebook user' : 'Instagram user'}</Typography>
                                                        {row?.phone_number &&
                                            ...
                                            <td>
                                                {
                                                    getFormattedChip(row?.attributes, 4)?.map(att => {
