...
export default function BroadcastRetryTable({ rows, isLoading, onRowClick, totalRows,
    currentPage, setCurrentPage, handleSearch, onEdit, onButtonClick, showRetry }) {
                ...
                >
                    <thead>
                        <tr>
                            ...
                            <th style={{ width: 60, padding: '12px 6px' }}>Name</th>
                            ...
                            <th style={{ width: 30, padding: '12px 6px' }}>Status</th>
                            <th style={{ width: 60, padding: '12px 6px' }}>Re-try</th>
                    ...
                    {isLoading ? loaderView :
                        rows?.length === 0 ? <tbody>
                            ...
                            :
                            <tbody>
                                {[...rows].sort(getComparator(order, 'createdAt')).map((row, index) => (
                                            ...
                                            <CircularWithValueLabel
                                                progress={
                                                    percentageCalculator(
                                                        ...
                                                        row?.messages?.filter(item => item?.double_tick_status !== 'failed' && item?.double_tick_status !== 'queued')?.length ?? 0
                                                    )
                                                }
                                            />
                                        </td>
                                        <td>
                                            <CircularWithValueLabel
                                                progress={
                                                    percentageCalculator(
                                                        row?.contacts?.length ?? 0,
                                                        row?.messages?.filter(item => item?.double_tick_status === 'read')?.length ?? 0
                                                    )
                                                }
                                            />
                                        </td> */}
